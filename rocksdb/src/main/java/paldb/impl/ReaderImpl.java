...
import paldb.api.NotFoundException;
import paldb.api.StoreReader;
...
//  = new DataInputOutput();
  ...
  private final StorageReader mainStorage;
  ...
  ReaderImpl(Configuration config, File file) {
   ...
   try {
        ...
        mainStorage = new StorageReader(config, file);
    } catch (IOException ex) {
      throw new RuntimeException(ex);
    }
    opened = true;
    int keyCount = mainStorage.getKeyCount();
    // Cache
    cache = StorageCache.initCache(config, keyCount);
  }
  ...
  public void close() {
    ...
    try {
      ...
      for (StorageReader storage: allStorageReader.values()) {
    	  storage.dupClose();
      }
      allStorageReader.clear();
      mainStorage.close();
      opened = false;
    } catch (IOException ex) {
      throw new RuntimeException(ex);
  ...
  public long size() {
    checkOpen();
    return localStorage.get().getKeyCount();
  }
  ...
  public <K, V> Iterable<Map.Entry<K, V>> iterable() {
    checkOpen();
    return new ReaderIterable(localStorage.get(), localSerialization.get());
  }

  @Override
  public <K> Iterable<K> keys() {
    checkOpen();
    return new ReaderKeyIterable<K>(localStorage.get(), localSerialization.get());
  }
