...
public class StorageCache {
   ...
   * @return new cache
   */
  static StorageCache initCache(Configuration configuration, int keyCount) {
    if (configuration.getBoolean(Configuration.CACHE_ENABLED) && configuration.getLong(Configuration.CACHE_BYTES) > 0) {
    	if (configuration.getBoolean(Configuration.GUAVA_ENABLED)) {
    		int size = keyCount;
    		if (configuration.getInt(Configuration.GUAVA_CACHE_SIZE) > 0 ) {
    			size = keyCount > configuration.getInt(Configuration.GUAVA_CACHE_SIZE) ?
    					configuration.getInt(Configuration.GUAVA_CACHE_SIZE): keyCount;
    		}
    		return new GuavaCache(configuration.getInt(Configuration.GUAVA_CONCURRENT_LEVEL), size);
    	} else {
  ...
  private static class GuavaCache extends StorageCache {
	 private Cache<Object, Object> cache;

	 public GuavaCache(int concurrentLevel, int cacheSize) {
		  this.cache = CacheBuilder.newBuilder().maximumSize(cacheSize)
                  .concurrencyLevel(concurrentLevel)
                  .initialCapacity(cacheSize).build();
	  }
	  @Override
	    public Object get(Object key) {
